function MysticSnakeInitialize( keys )
	...
	local bonus_damage = ability:GetLevelSpecialValueFor("snake_bonus_damage", ability_level) + caster:FindTalentValue("special_bonus_memethrow_volan_3")
	local damage = base_damage + (target:GetHealth() / 100) * bonus_damage
	
	local manasteal = ability:GetLevelSpecialValueFor("snake_manasteal", ability_level) + caster:FindTalentValue("special_bonus_memethrow_volan_4")
	local mana = (target:GetMana() / 100) * manasteal

	caster.mystic_snake_jumps = 1
	caster.mystic_snake_damage = damage
	caster.mystic_snake_mana = mana
